...
Float_t rapidityGapAlgorithm(TString input, Float_t etaEdg, Float_t trkPt[], Float_t trkEta[], Float_t ptThreshold, Float_t nTrk){

	Float_t etaBinNumber = 25;
	Float_t etaBin = 2*etaEdg/etaBinNumber;
	Float_t rightGap=0;
	Float_t leftGap =0;
	Bool_t hit;
	hit = false;
	...
	if(input.Contains("right")){
		for(Int_t j=1; j<etaBinNumber; j++){
			for(Int_t k=0; k<nTrk; k++){
				if(trkPt[k]>ptThreshold&&(trkEta[k]<etaEdg && trkEta[k]>(etaEdg-etaBin*j))) hit=true;
			}
			if( hit == false){
				rightGap+= etaBin;
			}
	...
	if(input.Contains("left")){
		for(Int_t j=1; j<etaBinNumber; j++){
			for(Int_t k=0; k<nTrk; k++){
				if(trkPt[k]>ptThreshold&&(trkEta[k]>-1.0*etaEdg && trkEta[k]<(-1.0*etaEdg+etaBin*j))) hit=true;
			}
			if( hit == false){
				leftGap+= etaBin;
			}
...
Bool_t applyEventSelection(UPCdiJet dj){
	if(dj.pt1>20 && dj.pt2>15 && dj.nJet==2){
		if( abs(dj.eta1)<1.8 && abs(dj.eta2)<1.8 ){
			if(dj.dphi>2.0){  //0.2){
				return true;
			}
			}
...
Float_t rapidityGapAlgorithmGEN(TString input, Float_t etaEdg, vector<float> *pt, vector<float> *eta, Float_t ptThreshold){
	Float_t etaBinNumber = 25;
	Float_t etaBin = 2*etaEdg/etaBinNumber;
	Float_t rightGap=0;
	Float_t leftGap =0;
	...
	if(input.Contains("right")){
		for(Int_t j=1; j<etaBinNumber; j++){
			for(Int_t k=0; k<eta->size(); k++){
				if( (*pt)[k] >ptThreshold&&( (*eta)[k]<etaEdg && (*eta)[k]>(etaEdg-etaBin*j))) hit=true;
			}
			if( hit == false){
				rightGap+= etaBin;
			}
	...
	if(input.Contains("left")){
		for(Int_t j=1; j<etaBinNumber; j++){
			for(Int_t k=0; k<eta->size(); k++){
				if( (*pt)[k] >ptThreshold&&( (*eta)[k]>-1.0*etaEdg && (*eta)[k]<(-1.0*etaEdg+etaBin*j))) hit=true;
			}
			if( hit == false){
				leftGap+= etaBin;
			}
...
TGraphErrors* getSystematics(TProfile* data, TProfile* wobble){
	Int_t n = data->GetSize()-1;
	Double_t x[n];
	Double_t y[n];
	Double_t ex[n];
	Double_t ey[n];
	for(Int_t i=1; i<n+1; i++){
		...
		Double_t error = abs(data->GetBinContent(i) - wobble->GetBinContent(i));
		x[i] = binCenter;
		y[i] = data->GetBinContent(i);
		ex[i] = (40-15)/10;
		ey[i] = error;
	}
...
TGraphErrors* getSystematicsFromHists(TH1D* data, TH1D* wobble){
	Int_t n = data->GetSize()-1;
	Double_t x[n];
	Double_t y[n];
	Double_t ex[n];
...
TH1D* getProfileDifference(TProfile* data, TProfile* mix){
	TH1D *h1 = (TH1D*)data->ProjectionX("h1");
	TH1D *h2 = (TH1D*)mix->ProjectionX("h2");
...
TH1F* getHistDifference(TH1F* data, TH1F* mix){
	TH1F *h3 = (TH1F*)data->Clone("h3");
	data->Sumw2();
	mix->Sumw2();
	h3->Sumw2();
	h3->SetTitle("h3");
...
TH1D* getProfileDividedByHist(TH1D* numerator, TProfile* denominator, const char* title){
	TH1D *h2 = (TH1D*)denominator->ProjectionX("");
	TH1D *h3 = (TH1D*)numerator->Clone(title);
	numerator->Sumw2();
	h2->Sumw2();
	h3->Sumw2();
	h3->SetTitle(title);
...
TGraphErrors* profileMoldSystematicErrors(TProfile* p1){
	Double_t x[5]  = {20.5, 25.5, 30.5, 35.5, 40.5};
   	Double_t y[5]  = {p1->GetBinContent(1),p1->GetBinContent(2),p1->GetBinContent(3),p1->GetBinContent(4),p1->GetBinContent(5)};
  	...
  	gr->SetTitle("V2 vs PT");
  	return gr;
}

TGraphErrors* histMoldSystematicErrors(TH1D* p1){
	Double_t x[5]  = {20.5, 25.5, 30.5, 35.5, 40.5};
   	Double_t y[5]  = {p1->GetBinContent(1),p1->GetBinContent(2),p1->GetBinContent(3),p1->GetBinContent(4),p1->GetBinContent(5)};
   	Double_t ex[5] = {1,1,1,1,1}; 
   	Double_t ey[5] = {0.022,0.023,0.022,0.028,0.035};
...
TCanvas* setUpTCanvas(const char* title){
	int W = 600;
	int H = 600;
	int W_ref = 600;
	int H_ref = 600;
	...
	float L = 0.15*W_ref;
	float R = 0.04*W_ref;
	TCanvas *c = new TCanvas(title,title,50,50,W,H);
	c->SetFillColor(0);
	c->SetBorderMode(0);
...
void formatTProfile(TProfile* p1, const char* xaxis, const char* yaxis, const char* bin, const char* gaps, const char* type){
	TPaveText *drtxt = new TPaveText(0.20,0.75,0.48,0.9,"brNDC"); 
	drtxt->AddText(type);
	drtxt->AddText(bin);
	drtxt->AddText(gaps);
	p1->GetXaxis()->SetTitle(xaxis);
	p1->GetYaxis()->SetTitle(yaxis);
	p1->GetYaxis()->SetNoExponent(kTRUE);
	p1->SetMinimum(-1);
	p1->SetMaximum(1);
...
void formatTH1D(TH1D* p1, const char* xaxis, const char* yaxis){
	p1->GetXaxis()->SetTitle(xaxis);
	p1->GetYaxis()->SetTitle(yaxis);
	p1->GetYaxis()->SetNoExponent(kTRUE);
	p1->SetMinimum(-1);
